...
async def async_setup_entry(hass: HomeAssistant, entry: AlfenConfigEntry):
    ...
    try:
        await hub.read_modbus_data_all()
...
class AlfenModbusHub:
    ...
    def async_add_alfen_sensor(self, update_callback, refresh_callback = None):
        ...
        if not self._sensors:
            ...
            self._hass.async_create_task(self.read_modbus_data_all())
    ...
    async def async_refresh_modbus_data(self, _now: int | None = None) -> None:
        ...
        try:
            """ Read socket every tick, read the rest every 12 ticks. """
            if self._tick % 12 == 0:
                update_result = await self.read_modbus_data_all()
            else:
                update_result = await self.read_modbus_data_socket_only()
        except Exception:
    ...
    async def read_modbus_data_all(self):
        if self._closing:
            return False
        return (
    ...
    async def read_modbus_data_socket_only(self):
        if self._closing:
            return False
        return (
